...
for cx = 1:length(condition)
    for idx = 1:length(ID)
        ...
        % Load the data
        filename = [ID{idx},'_FEF_Decaytest_withTMSPulse_step3_SOUND.set'];
        ...
        % Save the data
        filename = [ID{idx},'_FEF_Decaytest_withTMSPulse_step4_SOUND',condition{cx},'.set'];
        filepath = pathIn;
