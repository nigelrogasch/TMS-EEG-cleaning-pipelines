...
for idx = 1:length(ID)
    ...
    % TESA implementation
    EEG = pop_tesa_fitartifactmodel( EEG, 'tPulseMs', 0, 'tPulseDurationMs', 2, 'tSkipMs', 8.1, 'tSelectMs', 12, 't_fitmodel_p_ms', [0  20; 0 20; 0 20], 't_fitmodel_n_ms', [0  20; 0 20; 0 20], 't_lincomb_ms', [0  100], 'epoch_range', [-500  500] );

    % Save the data
    filename = [ID{idx},'_FEF_Decaytest_withTMSPulse_step3_FrecheModel.set'];
    filepath = pathIn;
    EEG = pop_saveset( EEG, 'filename',filename,'filepath',filepath);

end

%% PRE-TESA IMPLEMENTATION
...
% t_fitmodel_p = [];
% t_fitmodel_n = [];
% t_fitmodel_p{1} = 0:200;
% t_fitmodel_p{2} = 0:200;
% t_fitmodel_p{3} = 0:200;
% t_fitmodel_n{1} = 0:200;
% t_fitmodel_n{2} = 0:200;
% t_fitmodel_n{3} = 0:200;
% 
% t_lincomb = 0:1000;
% 
